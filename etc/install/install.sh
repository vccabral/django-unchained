...
sudo -u vagrant -s -- /usr/local/bin/virtualenv $VIRTUALENV_DIR --distribute
sudo -u vagrant -s -- echo $PROJECT_DIR > $VIRTUALENV_DIR/.project

cp -p $PROJECT_DIR/etc/install/bashrc /home/vagrant/.bashrc
echo "workon $VIRTUALENV_NAME" >> /home/vagrant/.bashrc
sudo -u vagrant -s -- /usr/bin/pip install -E $VIRTUALENV_DIR 
sudo -u vagrant -s -- /usr/bin/pip install -r $PROJECT_DIR/requirements.txt
